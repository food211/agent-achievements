...
- `run_wuxing_diagnostic`: load the sibling `wuxing-harness` Skill and start or resume its saved, step-by-step rule diagnosis. Before each question, scan the available rules, code, history, and decision database, save an evidence-backed candidate list, and show it to the user. Ask the user only to confirm, correct, exclude, or prioritize what the Agent cannot decide; never ask them to enumerate facts the Agent can inspect. Finish the creator, technical, and boundary phases in order. Do not silently modify rules that require human judgment.
